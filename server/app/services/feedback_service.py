        ...
        return [
            {
                ...
                "起点": fb.route_start,
                "终点": fb.route_end,
                "行程时间": fb.trip_time.strftime("%Y-%m-%d %H:%M:%S") if fb.trip_time else "",
                "行程时长": fb.trip_duration,
                "评分": fb.rating,
                "反馈内容": fb.feedback_text,
                "反馈类型": ",".join(str(t) for t in fb.feedback_type) if fb.feedback_type and isinstance(fb.feedback_type, list) else "",
                "情感": fb.sentiment or "",
                "关键词": ",".join(str(k) for k in fb.keywords) if fb.keywords and isinstance(fb.keywords, list) else "",
                "状态": fb.status,
                "处理人": fb.handler or "",
                "处理备注": fb.handler_notes or "",
                "AI摘要": fb.ai_summary or "",
                "反馈渠道": fb.feedback_channel,
                "创建时间": fb.created_at.strftime("%Y-%m-%d %H:%M:%S") if fb.created_at else "",
            }
            for fb in feedbacks
        ]
